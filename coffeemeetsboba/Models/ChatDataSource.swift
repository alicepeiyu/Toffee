...
//    }
    ...
    func addTextMessage(_ text: String, success:Bool) {
        let uid = "\(self.nextMessageId)"
        self.nextMessageId += 1
        let message = ChatMessageFactory.makeTextMessage(uid, text: text, isIncoming: false, success: success)
    ...
    func addIncomingTextMessage(_ text: String) {
        let uid = "\(self.nextMessageId)"
        self.nextMessageId += 1
        let message = ChatMessageFactory.makeTextMessage(uid, text: text, isIncoming: true, success: true)
        self.nextMessageId += 1
        self.slidingWindow.insertItem(message, position: .bottom)
        self.delegate?.chatDataSourceDidUpdate(self)
    }
